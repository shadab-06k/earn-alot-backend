import { internal, toNano, Address, StateInit } from "@ton/core";
...
export async function deployLotteryToTestnet(params: {
  ...
  if (mnemonic.length !== 24) {
    throw new Error(
      `Invalid mnemonic length. Expected 24 words, got ${mnemonic.length}. Please check your ADMIN_MNEMONIC format.`
    );
  }
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      client = new TonClient({
        endpoint,
        apiKey:
          "a2ac90f03c7c129df083afa916b29c19609e4a54cd0686dc75ff11631ebd41b3",
      });
  ...
  if (!client) {
    throw new Error(
      `Failed to connect to any TON endpoint. Last error: ${lastError?.message}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to find wallet with sufficient balance. Please check your mnemonic and ensure you have at least 0.1 TON in your testnet wallet. Error: ${versionError.message}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to get wallet sequence number: ${seqnoError.message}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to build deployment transaction: ${buildError.message}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to send deployment transaction: ${transferError.message}`
    );
  }
  return {
    contractAddr: contractAddr.toString(), // ✅ Convert to string for consistent response
